...
plot(bry.funct.pca, scaling=2, main="PCA - bryo traits", type="none", xlab=c("PCA1 (41.21%)"), ylab=c("PCA2 (20%)"), xlim=c(-1.3, 1.3), ylim=c(-1.35,1.35), cex = 1)
...
####Download SVG file for modification in Inkscape ####
svglite("figure_S6_bry_traits.svg", width = 7, height = 10)
grid.arrange(g.1, g.2, g.3, g.4,g.5,g.6,g.7,g.8,ncol=2)
dev.off()

###########################################
#### Plot functional traits as numeric ####
###########################################
...
#Select only traits used for calculation of FDis
traits_vasc <- dplyr::select(traits_vasc, -Espece, -Code)
...
#Modify trait names for plotting
colnames(cwm_vascu) <- c('Herbaceous', 'Shrub', 'Tree','Height_numeric','Seed_numeric','SLA_numeric', 'Seed 1', 'Seed 2', 'Seed 3','Height 1', 'Height 2', 'Height 3', 'SLA 1', 'SLA 2', 'SLA 3')
...
#Select only traits used for calculation of FDis
traits_bryo <- dplyr::select(traits_bryo, -Espece,-Code,-peristome)
...
#Modify trait names for plotting
colnames(cwm_bryo) <- c('Acrocarpous','Pleurocarpous','Sphagnum','Tuft','Turf','Weft','Shoot_numeric','Shoot 1','Shoot 2','Shoot 3','Dioicous','Monoicous','Seta 0','Seta 2','Seta 3','Seta 4','No peristome','Peristome perfect','Peristome specialized','Spore_numeric','Spore 1','Spore 2','Spore 3','No_Tomentum', 'Tomentum')
...
############################################
#### Numeric Vascular traits - latitude ####
############################################

env<-merge(env, cwm_vascu, by = "row.names") #merge trait frequencies (CWM matrix) with environmental data for easy plotting. 

v.2.1 <- ggplot(env, aes(x=Latitude, y=Height_numeric)) +
...
v.3.1<- ggplot(env, aes(x=Latitude, y=Seed_numeric)) +
  geom_smooth(method=lm, se=TRUE, formula = my.formula) +
  stat_poly_eq(formula = my.formula, aes(label = paste(..eq.label.., ..rr.label..,..p.value.label.., sep = "~~~")), parse = TRUE, size = 3)+
  geom_point(size=0.5)+
  labs(title = '',y="Seed weight", x = "Latitude (degrees)")+
...
v.4.1 <- ggplot(env, aes(x=Latitude, y=SLA_numeric)) +
  geom_smooth(method=lm, se=TRUE, formula = my.formula) +
  stat_poly_eq(formula = my.formula, aes(label = paste(..eq.label.., ..rr.label..,..p.value.label.., sep = "~~~")), parse = TRUE, size = 3)+
  geom_point(size=0.5)+
  labs(title = '',y="SLA", x = "Latitude (degrees)")+
...
m.3.1<- ggplot(env, aes(x=Latitude, y=Shoot_numeric)) +
  geom_smooth(method=lm, se=TRUE, formula = my.formula) +
  stat_poly_eq(formula = my.formula, aes(label = paste(..eq.label.., ..rr.label..,..p.value.label.., sep = "~~~")), parse = TRUE, size = 3)+
  geom_point(size=0.5)+
  labs(title = '',y="Shoot length", x = "Latitude (degrees)")+
...
m.6.1 <- ggplot(env, aes(x=Latitude, y=Spore_numeric)) +
  geom_smooth(method=lm, se=TRUE, formula = my.formula) +
  stat_poly_eq(formula = my.formula, aes(label = paste(..eq.label.., ..rr.label..,..p.value.label.., sep = "~~~")), parse = TRUE, size = 3)+
  geom_point(size=0.5)+
  labs(title = '',y="Spore size", x = "Latitude (degrees)")+
...
####Download SVG file for modification in Inkscape ####
svglite("figure_S7_num_traits.svg")
grid.arrange(v.2.1,v.3.1,v.4.1, m.3.1,m.6.1,ncol=3)
dev.off()
